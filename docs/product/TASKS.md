...
- [x] **Dossier de preuve (proof-of-compromise)** — **Done :** capture preuve publiée par l'attaquant (screenshot + métadonnées + option `.onion` live), SHA-256, horodatage RFC 3161, chaîne de custody ISO 27037, note légale FR (eIDAS/constat), + **identifiants de propagation** (magnet/infohash, hashes publiés, listing fichiers) pour tracer le dump ailleurs sans le télécharger, + **capture JS** (Playwright/Chromium via Tor : rend les leak sites SPA modernes + screenshot pleine page, fallback fetch statique). Métadonnées-only. · **Fait :** `evidence.py`
